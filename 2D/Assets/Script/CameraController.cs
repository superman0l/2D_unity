...
public class CameraController : MonoBehaviour
{
    ...
    void Start()
    {
        tf.position = new Vector3(playertransform.position.x, 1.2f, tf.position.z);
    }
    ...
    void CameraFollow() {
        if (tf.position.x >= limit_l && tf.position.x <= limit_r && playertransform.position.y >= limit_b - someground && playertransform.position.y <= limit_g - someground)
